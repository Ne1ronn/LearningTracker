...
async def ask_sort(cb: CallbackQuery):
    await cb.message.answer(
        "Choose the sorting method:", reply_markup=create_sort_buttons()
    )
    await cb.answer()
...
async def sort_date(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.answer(
        "Choose the sorting method:", reply_markup=create_asc_desc_buttons()
    )
    await cb.answer()
...
async def set_sort_progress(cb: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(sort=field)
    await cb.message.answer(
        "Sorting by progress is enabled ✅", reply_markup=ReplyKeyboardRemove()
    )
