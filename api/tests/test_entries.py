...
async def test_create_entry_ok(client, user_payload, entry_payload):
    ...
    authorized_response = await client.post(
        "/entries", json=entry_payload, headers=headers
    )
...
async def test_update_entry(client, user_payload, entry_payload):
    ...
    response = await client.put(
        f"/entries/{entry_id}", json=entry_update_payload, headers=headers
    )
...
async def test_patch_entry(client, user_payload, entry_payload):
    ...
    response = await client.patch(
        f"/entries/{entry_id}", json={"private": True}, headers=headers
    )
    assert response.status_code == 200
