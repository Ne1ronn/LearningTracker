...
async def create_topic_and_get_id(client, headers):
    ...
    assert len(topics) > 0
    topic_id = next((entry["id"] for entry in topics if entry["title"] == "FastAPI"), None)
...
async def test_update_topic(client, admin_headers):
    ...
    update_topic_response = await client.put(f"/topics/{topic_id}", json=update_topic_payload, headers=admin_headers)
...
async def test_patch_topic(client, admin_headers):
    ...
    patch_topic_response = await client.patch(f"/topics/{topic_id}", json={"is_active": False}, headers=admin_headers)
...
async def test_delete_topic(client, admin_headers):
    ...
    delete_topic_response = await client.delete(f"/topics/{topic_id}", headers=admin_headers)
